...
import {
  ...
  Clock, CheckCircle, Activity, Calendar,
} from "lucide-react";
...
const KpiCard = ({ icon: Icon, iconBg, iconColor, label, value, sub, subColor, evol, onClick }) => (
  <div
    className={`bg-white rounded-2xl border border-slate-100 shadow-sm p-5 ${onClick ? "cursor-pointer hover:shadow-md transition-shadow" : ""}`}
    onClick={onClick}
  >
...
const Dashboard = () => {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [dashData, socData] = await Promise.all([
  ...
  return (
    <div className="min-h-screen bg-slate-50/60 p-6 space-y-6">
      ...
      <div className="flex flex-wrap justify-between items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">
            {nomSoc} · {fmtDate(new Date())} · Actualisé à {lastRefresh.toLocaleTimeString("fr-FR")}
          </p>
        </div>
        <button onClick={loadData}
          className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition">
      ...
      {/* ── KPIs Row 1 : Clients / Commandes / Unités / CA ── */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard
          icon={Users} iconBg="bg-blue-50" iconColor="text-blue-600"
          label="Clients enregistrés" value={data.totalClients}
          onClick={() => navigate("/app/clients")}
        />
        <KpiCard
          icon={ShoppingCart} iconBg="bg-emerald-50" iconColor="text-emerald-600"
          label="Commandes actives" value={data.commandesActives}
          sub={`${data.commandesPending} en attente`}
          subColor={data.commandesPending > 0 ? "text-amber-500" : "text-slate-400"}
        ...
        <KpiCard
          ...
          sub={`${data.unitsLoues}/${data.totalUnits} unités louées`}
          onClick={() => navigate("/app/serials")}
        />
        <KpiCard
          icon={CreditCard} iconBg="bg-emerald-50" iconColor="text-emerald-600"
          label="CA ce mois" value={fmtMad(data.caMois)}
          evol={data.caEvol}
          sub={`Total : ${fmtMad(data.caTotal)}`}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard
          ...
          label="Factures impayées" value={data.facturesImpayees.length}
          sub={fmtMad(data.totalRestant) + " restant"}
          subColor="text-red-500"
          onClick={() => navigate("/app/facturation")}
        />
        <KpiCard
          icon={Clock} iconBg="bg-amber-50" iconColor="text-amber-600"
          label="Paiements en attente" value={data.paiementsPending}
          onClick={() => navigate("/app/paiements")}
        />
        <KpiCard
          icon={Calendar} iconBg="bg-orange-50" iconColor="text-orange-600"
          label="Contrats expirant (7j)" value={data.commandesARenouveler.length}
          subColor="text-orange-500"
          sub={data.commandesARenouveler.length > 0 ? "À renouveler" : "Aucun"}
          onClick={() => navigate("/app/commandes")}
        />
        <KpiCard
          icon={Activity} iconBg="bg-slate-50" iconColor="text-slate-600"
          label="Unités en maintenance" value={data.unitsMaintenance}
          subColor={data.unitsMaintenance > 0 ? "text-orange-500" : "text-slate-400"}
          sub={data.unitsMaintenance > 0 ? "Attention requise" : "Tout est OK"}
          onClick={() => navigate("/app/serials")}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          ...
          {data.recentCommandes.length === 0 ? (
            <div className="p-8 text-center text-slate-400 text-sm">Aucune commande</div>
          ) : (
                ...
                {data.recentCommandes.map((cmd) => (
                  <tr key={cmd._id} className="hover:bg-slate-50 cursor-pointer" onClick={() => navigate("/app/commandes")}>
                    ...
                    <td className="px-4 py-2.5 font-medium text-slate-800 text-xs">
                      {cmd.client ? `${cmd.client.prenom || ""} ${cmd.client.nom || ""}`.trim() : "—"}
                    </td>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          ...
          {data.recentPaiements.length === 0 ? (
            <div className="p-8 text-center text-slate-400 text-sm">Aucun paiement</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-slate-400 text-[10px] uppercase">
                <tr>
              ...
              <tbody className="divide-y divide-slate-50">
                {data.recentPaiements.map((p) => (
                  <tr key={p._id} className="hover:bg-slate-50 cursor-pointer" onClick={() => navigate("/app/paiements")}>
                    ...
                    <td className="px-4 py-2.5 font-medium text-slate-800 text-xs">
                      {p.client ? `${p.client.prenom || ""} ${p.client.nom || ""}`.trim() : "—"}
                    </td>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-100 flex justify-between items-center">
            <h3 className="text-sm font-bold text-slate-800">
              🧾 Factures impayées
              <span className="ml-2 px-2 py-0.5 rounded-full text-xs bg-red-100 text-red-600 font-bold">{data.facturesImpayees.length}</span>
            </h3>
              ...
              {data.facturesImpayees.map((f) => {
                ...
                return (
                  <tr key={f._id} className="hover:bg-slate-50 cursor-pointer" onClick={() => navigate("/app/facturation")}>
                    ...
                    <td className={`px-4 py-2.5 text-xs ${isLate ? "text-red-600 font-bold" : "text-slate-500"}`}>
                      {fmtDate(f.dateEcheance)} {isLate && "⚠️"}
                    </td>
