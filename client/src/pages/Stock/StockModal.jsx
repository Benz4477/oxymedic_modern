...
const COLOR_PALETTE = [
  "#16A34A", "#2563EB", "#7C3AED", "#0891B2", "#D97706",
  "#DC2626", "#059669", "#9333EA", "#0F766E", "#B45309",
  "#BE185D", "#EA580C", "#374151", "#0369A1", "#15803D",
];
...
const StockModal = ({
  ...
  // ── Fonction de calcul automatique des prix ──
  const updatePrices = (type, value) => {
    const numValue = parseInt(value) || 0;
    
    if (type === "pDay") {
      // Si on change le prix jour, on calcule semaine (×7) et mois (×30)
      setFormData({
        ...formData,
        pDay: numValue,
        pWeek: numValue * 7,
      ...
      // Si on change le prix semaine, on calcule jour (÷7) et mois (×4)
      setFormData({
        ...formData,
        pWeek: numValue,
        pDay: Math.round(numValue / 7),
        pMonth: numValue * 4,
      });
    } else if (type === "pMonth") {
      // Si on change le prix mois, on calcule semaine (÷4) et jour (÷30)
      setFormData({
        ...formData,
        pMonth: numValue,
        pWeek: Math.round(numValue / 4),
        pDay: Math.round(numValue / 30),
      });
    }
  };
  ...
  // Listes prédéfinies pour Marque et Origine
  const marqueList = [
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ex: OXY-MOB-001"
              />
              <p className="text-[10px] text-slate-400 mt-1">Code unique du produit (OXY-[CAT]-[NUM])</p>
            </div>

            {/* Marque + Origine avec datalist */}
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1.5">
                  🏷️ Marque
                ...
                <input
                  ...
                  placeholder="Ex: Philips, Invacare..."
                />
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ex: Entrepôt A — Rayon 1"
              />
              <p className="text-[10px] text-slate-400 mt-1">Localisation physique dans l'entrepôt</p>
            </div>

            {/* Tarifs (jour, semaine, mois) avec calcul automatique */}
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1.5">
                💰 Tarifs de location (MAD)
              </label>
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-[9px] text-slate-500">Jour</label>
                  <input
                    type="number"
                    className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm"
                    value={formData.pDay}
                    onChange={(e) => updatePrices("pDay", e.target.value)}
                    placeholder="Prix par jour"
                  />
                  <p className="text-[9px] text-slate-400 mt-0.5">→ auto calcule semaine et mois</p>
                </div>
                <div>
                  <label className="block text-[9px] text-slate-500">Semaine</label>
                  <input
                    type="number"
                    className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm bg-slate-50"
                    value={formData.pWeek}
                    onChange={(e) => updatePrices("pWeek", e.target.value)}
                    placeholder="Prix par semaine"
                  />
                  <p className="text-[9px] text-slate-400 mt-0.5">→ auto recalcule jour et mois</p>
                </div>
                <div>
                  <label className="block text-[9px] text-emerald-600 font-bold">Mois ★</label>
                  <input
                    type="number"
                    className="w-full border border-emerald-200 rounded-xl px-3 py-2 text-sm bg-emerald-50"
                    value={formData.pMonth}
                    onChange={(e) => updatePrices("pMonth", e.target.value)}
                    placeholder="Prix par mois (affiché)"
                  />
                  <p className="text-[9px] text-emerald-600 mt-0.5">→ auto recalcule semaine et jour</p>
                </div>
              </div>
              <p className="text-[10px] text-slate-400 mt-2">💡 Saisissez un prix dans n'importe quel champ – les autres se mettront à jour automatiquement</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, pVente: parseInt(e.target.value) || 0 })}
                  placeholder="Prix d'achat pour le client"
                />
              </div>
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1.5">
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, caution: parseInt(e.target.value) || 0 })}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, dispo: parseInt(e.target.value) || 0 })}
                  placeholder="Nombre disponible à la location"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="3700000000000 (EAN-13 ou CODE128)"
                />
                <button
                  type="button"
                  ...
                  onClick={() =>
                    setFormData({
                      ...formData,
                      productBarcode: "370" + Math.floor(Math.random() * 1e10).toString().padStart(10, "0"),
                    })
                  }
                >
                  ⚡ Générer
              ...
              <p className="text-[10px] text-slate-400 mt-1">Ce code sera automatiquement proposé lors de la création des unités (N° série)</p>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Caractéristiques techniques, informations complémentaires..."
              />
              <p className="text-[10px] text-slate-400 mt-1">Description détaillée du produit (caractéristiques, utilisation, etc.)</p>
            </div>
