...
const Commandes = () => {
  ...
  const [bonEnlevementCmd, setBonEnlevementCmd] = useState(null);
  const [bonRetourCmd, setBonRetourCmd]         = useState(null);
    ...
    <div className="min-h-screen bg-slate-50/60 p-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <CommandeTable
          ...
          onBonEnlevement={(cmd) => setBonEnlevementCmd(cmd)}
          onBonRetour={(cmd) => setBonRetourCmd(cmd)}
        />
      </div>
      ...
      {bonEnlevementCmd && (
        <BonEnlevementModal
          commande={bonEnlevementCmd}
          societe={societe}
          onClose={() => setBonEnlevementCmd(null)}
        />
      )}
      ...
      {bonRetourCmd && (
        <BonRetourModal
          commande={bonRetourCmd}
          societe={societe}
          onClose={() => setBonRetourCmd(null)}
        />
      )}
    </div>
  );
