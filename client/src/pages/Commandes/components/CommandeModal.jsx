  ...
  useEffect(() => {
    if (formData.equipId) {
      const available = units.filter((u) => {
        // Gérer les différents formats d'equipId (string, ObjectId, objet)
        const targetEquipId =
          typeof formData.equipId === "object"
            ? formData.equipId._id ||
              formData.equipId.value ||
              String(formData.equipId)
            : String(formData.equipId);

        const unitEquipId =
          typeof u.equipId === "object"
            ? u.equipId._id || u.equipId.value || String(u.equipId)
            : String(u.equipId);

        const equipIdMatch = unitEquipId === targetEquipId;
        const isAvailable = u.status === "available" || u.status === "rented";

        return equipIdMatch && isAvailable;
      });
      ...
      if (available.length > 0 && !formData.unitId) {
        ...
        setFormData((prev) => ({ ...prev, unitId: firstAvailable._id }));
      } else if (available.length === 0) {
        // Réinitialiser unitId si aucune unité disponible
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              >
                ...
                <option>Cash livr.</option>
                <option>Cash mag.</option>
              </select>
            </div>
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    amountTTC: parseInt(e.target.value) || 0,
                  })
                }
              />
            </div>
          ...
          <div className="border border-amber-200 rounded-xl p-3 bg-amber-50/30">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  type="number"
                  className="w-full border border-amber-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-amber-400 outline-none"
              ...
              <div>
                ...
                <select
                  className="w-full border border-amber-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-amber-400 outline-none"
                  value={formData.cautionMode}
                  onChange={(e) =>
                    setFormData({ ...formData, cautionMode: e.target.value })
                  }
                >
                  <option>Cash</option>
